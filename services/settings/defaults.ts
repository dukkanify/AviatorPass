/**
 * Default platform settings — AviatorPass official brand guidelines.
 * Palette: Aviator Blue · Aviator Gold · Academic Grey.
 * Typography: Stimulatio Flat pending licensed files (Exo 2 substitute) + IBM Plex Sans.
 */

import { siteStatic } from "@/config/site-static";
import type { PlatformSettings } from "@/types/settings";

export const DEFAULT_PLATFORM_SETTINGS: PlatformSettings = {
  general: {
    platformName: "Aviator Pass",
    companyName: "Aviator Pass",
    websiteUrl: "https://www.aviatorpass.com",
    contactEmail: siteStatic.contactEmail,
    contactPhone: "",
    supportEmail: siteStatic.supportEmail,
    defaultTimezone: "Asia/Kuwait",
    defaultLanguage: "en",
    dateFormat: "MMM d, yyyy",
    timeFormat: "24h",
    currency: "KWD",
    country: "KW",
    maintenanceMode: false,
    platformStatus: "online",
    primaryLocations: ["Dubai", "Copenhagen", "Kuwait", "Qatar"],
    footerText:
      "Complete aviation education platform — live and recorded programmes with EASA Certified Instructors.",
    socialHandle: siteStatic.socialHandle,
    socialLinks: {
      instagram: siteStatic.social.instagram,
      twitter: siteStatic.social.twitter,
      linkedin: siteStatic.social.linkedin,
      youtube: siteStatic.social.youtube,
    },
  },
  branding: {
    logoUrl: "/brand/logo.png?v=brand-guide-4",
    darkLogoUrl: "/brand/logo-dark.png?v=brand-guide-4",
    faviconUrl: "/brand/favicon.svg?v=brand-guide-4",
    loginBackgroundUrl: "/images/hero-aviation.svg",
    loginIllustrationUrl: "/images/hero-aviation.svg",
    openGraphImageUrl: "/brand/og.png?v=brand-guide-4",
    primaryColor: "#143048",
    secondaryColor: "#7C7B80",
    accentColor: "#CCA04C",
    typographyDisplay: "Exo 2",
    typographyBody: "IBM Plex Sans",
    footerInformation:
      "Aviator Pass — YOUR AVIATION JOURNEY STARTS HERE. Complete aviation education across Dubai, Copenhagen, Kuwait, and Qatar.",
    brandGuidelinesPending: false,
    colorPalettePending: false,
    typographyPending: true,
    styleGuidePending: false,
  },
  email: {
    provider: "smtp",
    smtpHost: "",
    smtpPort: 587,
    smtpUsername: "",
    smtpPassword: "",
    encryption: "tls",
    senderName: "Aviator Pass",
    senderEmail: siteStatic.supportEmail,
    replyToEmail: siteStatic.supportEmail,
  },
  notifications: {
    emailNotifications: true,
    inAppNotifications: true,
    reminderEmails: true,
    marketingEmails: false,
    systemAlerts: true,
    classReminderOffsetsMinutes: [1440, 120],
    classReminderFifteenMinutesEnabled: true,
  },
  authentication: {
    otpExpirationMinutes: 10,
    otpMaxAttempts: 5,
    otpResendCooldownSeconds: 60,
    otpMaxResends: 5,
    otpLockoutMinutes: 15,
    passwordPolicyEnabled: true,
    minimumPasswordLength: 8,
    requireUppercase: true,
    requireNumber: true,
    requireSpecialChar: false,
    sessionTimeoutMinutes: 60 * 24 * 7,
    rememberMeDays: 30,
    maxLoginAttempts: 5,
    accountLockDurationMinutes: 30,
  },
  users: {
    defaultUserRole: "student",
    instructorApprovalRequired: false,
    studentApprovalRequired: false,
    emailVerificationRequired: true,
    phoneVerificationRequired: false,
  },
  security: {
    rateLimitingEnabled: true,
    rateLimitRequestsPerMinute: 60,
    ipBlockingEnabled: false,
    blockedIps: [],
    trustedDomains: [
      "localhost",
      "aviatorpass.com",
      "www.aviatorpass.com",
      "dubai-test.blog",
      "www.dubai-test.blog",
    ],
    allowedFileTypes: ["image/png", "image/jpeg", "image/webp", "image/svg+xml", "application/pdf"],
    maxUploadSizeMb: 10,
    twoFactorAuthEnabled: false,
    twoFactorAuthReady: true,
    singleDeviceLogin: true,
    preventAccountSharing: true,
    deviceFingerprintingEnabled: true,
    maxConcurrentSessions: 1,
    studentOnlyDeviceLock: true,
    contentWatermarkEnabled: true,
    disableRightClickOnLearning: true,
    blockScreenshotShortcuts: true,
    deterScreenRecording: true,
    videoDownloadProtection: true,
  },
  storage: {
    provider: "local",
    supabaseBucket: "aep-uploads",
    allowedExtensions: [".png", ".jpg", ".jpeg", ".webp", ".svg", ".pdf", ".ai"],
    storageQuotaGb: 50,
    automaticCleanupEnabled: false,
    cleanupOlderThanDays: 90,
  },
  localization: {
    timezone: "Asia/Kuwait",
    country: "KW",
    language: "en",
    regionalFormatting: "en-KW",
    availableLanguages: ["en"],
    englishOnly: true,
  },
  features: {
    blog: true,
    communities: true,
    certificates: true,
    payments: true,
    zoom: true,
    advertisements: false,
    wallet: true,
    courses: true,
    calendar: true,
    ai: true,
    mockExams: true,
    cgi: true,
    schedule: true,
    installments: true,
    emailAutomation: true,
    messaging: true,
    reports: true,
    mobileApps: false,
    corporatePortal: false,
    multiTenant: false,
    aiProctoring: false,
    learningPaths: false,
    crmIntegration: false,
    erpIntegration: false,
    marketingAutomation: false,
    enterpriseSso: false,
    i18n: false,
    whiteLabel: false,
    biPredictive: false,
  },
  zoom: {
    enabled: true,
    accountEmail: siteStatic.supportEmail,
    defaultWaitingRoom: true,
    defaultPasscode: true,
    defaultMeetingType: "meeting",
    credentialsConfigured: false,
  },
  courses: {
    publicDeliveryFilter: "all",
  },
  updatedAt: new Date(0).toISOString(),
  updatedBy: null,
};
