/**
 * Public Tamara / order session lookup — no secrets.
 */
...
export function getPublicTamaraSession(sessionId: string): PublicCheckoutSession {
    ...
    payments.find(
      (p) =>
        ...
        (p.providerPaymentId === id ||
          ...
          p.orderId === id ||
          String(p.rawProviderPayload.orderId ?? "") === id ||
          String(p.rawProviderPayload.checkoutId ?? "") === id),
  ...
  return {
    ...
    currency: order?.currency ?? resolvedPayment?.currency ?? "SAR",
